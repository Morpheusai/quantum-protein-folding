...
def main():
    ...
    parser.add_argument("--backend", type=str, default="aer_sim",
                        choices=["aer_sim", "aws_sv1", "aws_garnet", "aws_forte"],
                        help="Quantum computing backend (default: aer_sim)")
    ...
    if backend_mode != "aer_sim":
        print(f"\n[AWS] Using AWS Braket backend: {backend_mode}")
    else:
        print(f"\n[Local] Using local Aer simulator")

    # Optimize CVaR (multi-start)
    print(f"\n[CVaR-VQE] alpha={args.alpha}, tries={args.tries}")
    best_x, best_cvar, trace, _tries_info = optimize_cvar_multistart(hyper, args.tries, args.alpha, use_gpu=use_gpu)
...
"""
        ...
        # 3. Test sampling on selected backend
        if backend_mode != "aer_sim" and BraketProvider:
            try:
                print(f"\n[{backend_mode}] Testing sampling on AWS Braket...")
                qc_meas = build_scalable_ansatz(best_x, hyper, measure=True)
                provider = BraketProvider()
                if backend_mode == 'aws_sv1':
                ...
                result = backend.run(transpile(qc_meas, backend), shots=args.shots).result()
                counts = result.get_counts()
                print(f"[{backend_mode}] ✓ Sampling completed, got {len(counts)} unique states")
            except Exception as e:
                print(f"[{backend_mode}] ✗ Sampling failed:", e)

        # 4. Histogram of >= export_p using exact probs
        ...
        # 5. CSV: all states with energy and export flag
        ...
        # 6. Generate 3D PDBs for high-probability bitstrings
        try:
        ...
        # 8. Energy breakdown for the most negative bitstring
        plot_energy_breakdown_for_most_negative(probs, hyper, out_dir)

        # 9. CVaR scatter plot across iterations
