...
def read_cli_inputs():
    ...
    backend_str = ask("QC backend [DEFAULT aer_sim, ELSE: aws_sv1, aws_garnet, aws_forte]: ", f"{BACKEND_DEAULT}")
    try:
        backend_mode = backend_str
        if backend_mode not in ["aer_sim", "aws_sv1", "aws_garnet", "aws_forte"]: raise ValueError
    except Exception:
