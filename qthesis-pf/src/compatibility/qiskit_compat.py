...
def patch_diagonal_estimator():
    ...
    try:
        ...
        diagonal_estimator = importlib.import_module('qiskit_algorithms.minimum_eigensolvers.diagonal_estimator')
        ...
        def patched_run_pub(self, pub):
            ...
            for index in np.ndindex(*bc_circuits.shape):
                ...
                # 检查电路是否已有测量指令
                has_measurements = any(inst.operation.name == 'measure' for inst in bound_circuit.data)
            ...
            for sampler_pub_result, index in zip(sampler_pubs_results, np.ndindex(*bc_circuits.shape)):
                ...
                if hasattr(sampler_pub_result.data, 'meas'):
                    ...
                    for key in sampler_pub_result.data._data:
                        ...
                        if hasattr(value, 'get_int_counts') and hasattr(value, 'num_shots'):
                            meas_data = value
                            break
                ...
                # 确保我们有必需的方法
                if not hasattr(meas_data, 'get_int_counts') or not hasattr(meas_data, 'num_shots'):
                ...
                # 继续原始逻辑的其余部分
                evaluated = {
                    state: (probability, diagonal_estimator._evaluate_sparsepauli(state, obs))
