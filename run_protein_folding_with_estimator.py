...
def main():
    ...
    try:
        ...
        if quantum_backend.lower() == 'aws_sv1':
            # 使用AWS SV1模拟器
            try:
                from qiskit_braket_provider import BraketLocalBackend, BraketProvider, BraketEstimator
                backend = BraketLocalBackend()
                estimator = BraketEstimator(backend)
                print("  - 使用 AWS Braket 本地模拟器（Estimator）")
            ...
            try:
                ...
                print("  - 使用 IBM 量子服务（Estimator）")
            except Exception as e:
                print(f"  - IBM连接失败，使用本地模拟器: {e}")
                return
        else:
            from qiskit_aer import Aer 
            from qiskit.primitives import BackendEstimatorV2  # 使用 Estimator！
            backend = Aer.get_backend('qasm_simulator')
            estimator = BackendEstimatorV2(backend=backend)
            print("  - 使用本地 Qiskit 模拟器（Estimator）")
            print("  - 使用本地Qiskit模拟器")
        ...
        print(f"Estimator type: {type(estimator)}")  # 注意这里是 Estimator

        # 初始化VQE
        vqe = VQE(
            estimator=estimator,  # 使用 estimator 而不是 sampler
            ansatz=ansatz,
            optimizer=optimizer,
            aggregation=0.1,
